        ...
        public async Task<IHttpActionResult> Post([FromBody]WebHookMessage webHookMessage)
        {
            if (webHookMessage.Type == "EmotionUpdate")
            {
                ...
                using (var client = new ConnectorClient(new Uri("https://skype.botframework.com")))
                {
                    ...
                    var ConversationId = await client.Conversations.CreateDirectConversationAsync(new ChannelAccount(from), new ChannelAccount(to));
                    IMessageActivity message = Activity.CreateMessageActivity();
                    message.From = new ChannelAccount(from);
                    message.Recipient = new ChannelAccount(to);
